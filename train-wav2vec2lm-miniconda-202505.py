...
csvValid = sys.argv[3]
csvTest = sys.argv[4]
filenameCorrectKenlmModel= sys.argv[5]
folderLogFiles = sys.argv[6]
folderModelFiles = sys.argv[7]
runId = sys.argv[8]
desiredTrainEpochs = sys.argv[9]
outputPrefix = sys.argv[10]

print("asrLang: " + asrLang)
...
for ch in checkpointNums:
	...
	for m in modes:
		...
		filename = asrLang + "-wav2vec2-"+"res-" + str(runId) + "-ch" + str(ch) + "-" + m + ".csv"
		idThisRun = "wav2vec2-" + str(runId) + "-" + m

		output = "wav,src,res,loss,charDist,charLen,wordDist,wordLen,cer,wer,origin,id,typeMonoTri,ngram\n"
		cerList = []
		werList = []
		...
		for i in range(0,len(reference)):
			...
			output += wavFile + "," + reference[i] + "," + modePredictions[i] + ",," + str(charDist) + "," + str(charLen) + "," + str(wordDist) + "," + str(wordLen) + "," + str(round(cer,2)) + "," + str(round(werSent,2)) + "," + m + "-" + str(ch) + "," + str(idThisRun) + "," + "na" + "," + "na" + "\n"
